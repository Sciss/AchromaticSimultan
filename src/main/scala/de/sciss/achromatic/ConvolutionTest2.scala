...
// variant using sampled grid of positions
object ConvolutionTest2 {
  type S = InMemory
  ...
  def run(gui: Boolean = true, verb: Boolean = true, distant: Boolean = true): Future[(Double, Double) => Unit] = {
    ...
    Server.run { s =>
      ...
      val df = SynthDef("test") {
        import ugen._
        val bufIr   = "bufIr"   .ir
        val bufIdx  = "bufIdx"  .ir
        val bufDisk = "bufDisk" .ir
        val azi     = (( "azi".ar               % 360.0) * S).roundTo(1)
        val ele     = ((("ele".ar + (90.0: GE)) % 180.0) * S).roundTo(1)

//        azi.poll(2, "azi*S")
//        ele.poll(2, "ele*S")

        val indexP  = azi + ele * (360 * S)
//        indexP.poll(2, "indexP")
        val index0  = Index.ar(bufIdx, indexP)
...
//        val index0  = "index".ar(0f)
      ...
      Buffer.read(s, path = Common.fAudioIR.path, completion = { bufIr: Buffer =>
        Buffer.read(s, path = Common.fAudioIdxG.path, completion = { bufIdx: Buffer =>
          syn.play("test", args = Seq("bufIr" -> bufIr.id, "bufIdx" -> bufIdx.id, "bufDisk" -> bufDisk.id))
        })
        ()
      })

      val setPosF = { (azi: Double, ele: Double) =>
        syn.set("azi" -> azi, "ele" -> ele)
      }
